...
#define PROV_TIMEOUT_US (300ULL * 1000ULL * 1000ULL)
...
static const esp_gatts_attr_db_t s_gatt_db[IDX_NB] = {
	...
	[IDX_CHAR_SSID] = {{ESP_GATT_AUTO_RSP}, {ESP_UUID_LEN_16, (uint8_t *)&CHAR_DECL_UUID, ESP_GATT_PERM_READ, sizeof(uint8_t), sizeof(uint8_t), (uint8_t *)&CHAR_PROP_READ_WRITE}},
	[IDX_CHAR_VAL_SSID] = {{ESP_GATT_RSP_BY_APP}, {ESP_UUID_LEN_128, (uint8_t *)CHAR_SSID_UUID_128, ESP_GATT_PERM_WRITE_ENC_MITM, sizeof(s_ssid_value), 0, s_ssid_value}},
	[IDX_CHAR_PASS] = {{ESP_GATT_AUTO_RSP}, {ESP_UUID_LEN_16, (uint8_t *)&CHAR_DECL_UUID, ESP_GATT_PERM_READ, sizeof(uint8_t), sizeof(uint8_t), (uint8_t *)&CHAR_PROP_WRITE_ONLY}},
	[IDX_CHAR_VAL_PASS] = {{ESP_GATT_RSP_BY_APP}, {ESP_UUID_LEN_128, (uint8_t *)CHAR_PASS_UUID_128, ESP_GATT_PERM_WRITE_ENC_MITM, sizeof(s_pass_value), 0, s_pass_value}},
	[IDX_CHAR_CFG] = {{ESP_GATT_AUTO_RSP}, {ESP_UUID_LEN_16, (uint8_t *)&CHAR_DECL_UUID, ESP_GATT_PERM_READ, sizeof(uint8_t), sizeof(uint8_t), (uint8_t *)&CHAR_PROP_WRITE_ONLY}},
	[IDX_CHAR_VAL_CFG] = {{ESP_GATT_RSP_BY_APP}, {ESP_UUID_LEN_128, (uint8_t *)CHAR_CFG_UUID_128, ESP_GATT_PERM_WRITE_ENC_MITM, sizeof(s_cfg_value), 0, s_cfg_value}},
	[IDX_CHAR_MQTT_PASS] = {{ESP_GATT_AUTO_RSP}, {ESP_UUID_LEN_16, (uint8_t *)&CHAR_DECL_UUID, ESP_GATT_PERM_READ, sizeof(uint8_t), sizeof(uint8_t), (uint8_t *)&CHAR_PROP_WRITE_ONLY}},
	[IDX_CHAR_VAL_MQTT_PASS] = {{ESP_GATT_RSP_BY_APP}, {ESP_UUID_LEN_128, (uint8_t *)CHAR_MQTT_PASS_128, ESP_GATT_PERM_WRITE_ENC_MITM, sizeof(s_mqtt_pass_value), 0, s_mqtt_pass_value}},
	[IDX_CHAR_ESP_MAC] = {{ESP_GATT_AUTO_RSP}, {ESP_UUID_LEN_16, (uint8_t *)&CHAR_DECL_UUID, ESP_GATT_PERM_READ, sizeof(uint8_t), sizeof(uint8_t), (uint8_t *)&CHAR_PROP_READ_ONLY}},
	[IDX_CHAR_VAL_ESP_MAC] = {{ESP_GATT_AUTO_RSP}, {ESP_UUID_LEN_128, (uint8_t *)CHAR_ESP_MAC_128, ESP_GATT_PERM_READ_ENC_MITM, sizeof(s_esp_mac_firstboot_value), 0, s_esp_mac_firstboot_value}},
};
...
static void apply_if_complete(void)
{
	...
	const bool is_repair = app_context_has_prior_connect();
	const EventBits_t required_bits = is_repair
										  ? PROV_BIT_MQTT
										  : PROV_EVENT_MASK;
	...
	if (!is_repair || (bits & PROV_BIT_SSID))
	{
		memset(cfg.ssid, 0, sizeof(cfg.ssid));
		memcpy(cfg.ssid, s_ssid_value, ssid_len);
	}
	if (!is_repair || (bits & PROV_BIT_PASS))
	{
	...
	if (!is_repair || (bits & PROV_BIT_MQTT))
	{
...
static void handle_write_evt(esp_gatt_if_t gatts_if, esp_ble_gatts_cb_param_t *param)
{
	...
	{
		if (param->write.len == 0U && app_context_has_prior_connect())
		{
			ESP_LOGI(TAG, "ssid empty on re-pairing; keeping existing");
			send_write_response(gatts_if, param, ESP_GATT_OK);
			return;
		}
	...
	if (handle == s_handle_table[IDX_CHAR_VAL_PASS])
	{
		if (param->write.len == 0U && app_context_has_prior_connect())
		{
			ESP_LOGI(TAG, "pass empty on re-pairing; keeping existing");
			send_write_response(gatts_if, param, ESP_GATT_OK);
			return;
		}
