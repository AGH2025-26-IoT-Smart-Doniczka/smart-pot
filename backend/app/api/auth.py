...
def register(data: RegisterRequest):
    ...
    try:
        user = create_user(email=data.email, username=data.username, password_hash=password_hash)
    except IntegrityError:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail={"message": "User was not created", "error": "Email already in use"},
        )
    except Exception:
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail={"message": "User was not created", "error": "Internal server error"},
        )
